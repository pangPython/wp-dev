 ...
 * @since      1.0.0
 */
class boke_1_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => __( '显示最新的文章（带缩略图）', 'boke-1' )
		);

		// Create the widget.
		parent::__construct(
			'revenue_pro-recent',                                   // $this->id_base
			__( '&raquo; 最新的文章', 'boke-1' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'boke_1_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'boke_1_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'boke_1_recent_widget_' . $this->id );
	}
